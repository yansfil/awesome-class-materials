...
프로그래밍 패러다임은 프로그래밍할 때의 갖게되는 관점을 말합니다. 예를 들어 프로그래밍을 함수적 호출 과정을 중심으로 바라보고 설계한다면 절차 지향 프로그래밍이 되고, 객체의 책임과 협력을 중심으로 설계한다면 객체 지향 프로그래밍이 됩니다.

프로그래밍 패러다임은 프로그래밍 언어에도 반영되는데, 이를테면 C언어는 대표적인 절차지향 언어이며, 자바는 대표적인 객체지향 언어입니다. 이 두 패러다임을 모두 수용하기도 하는데, 파이썬이 이런 멀티 패러다임 언어라고 볼 수 있습니다.
...
절차지향(Procedural Orient) 프로그래밍은 프로시저 콜, 즉 함수 호출을 중심으로 프로그래밍을 생각하는 것입니다. 재사용 가능한 코드들은 별도의 함수로 분리하고 함수들간의 호출로 하고자 하는 일을 수행합니다. 이런 프로세스는 주로 "함수"와 "조건문", "루프문" 을 활용하여 코드를 구성합니다.
...
def get_txt_file_reader(file_type: str) -> Reader:
    if file_type == "txt":
   		...
    elif file_type == "csv":
        ...
...
그러나 절차지향은 전체 로직이 매우 복잡하거나 동적으로 로직을 바꿔야 하는 등의 기능 확장이 필요할 때 유지보수하기 어려워집니다. 특히 조건문이 추가적으로 계속해서 여기저기 붙어야할 가능성이 커집니다. 따라서 절차지향은 프로그램이 수행하는 알고리즘이 명확하고, 기능 확장 등이 자주 일어나지 않는 상황에서 사용하기에 좋습니다.
...
객체지향(Object Orient) 프로그래밍은 객체라고 하는 단위에 책임을 명확히 하고 이 객체들의 협력 중심으로 프로그래밍을 생각하는 것입니다. 모든 것을 이 객체로 나누어 생각하고, 필요할 때 이런 객체들을 재사용하여 하고자 하는 일을 수행합니다.

또한 절차지향과 다르게 객체가 자신의 일을 수행하는데 필요한 데이터는 자신이 모두 가지고 있습니다. 이러한 데이터는 외부에 공개할 필요가 없거나 해서는 안되는 데이터라면 모두 자신 내부에 숨겨 외부에서 알지 못하도록 합니다.
...
그리고 각 객체는 자신의 기능을 수행하는데 필요한 데이터를 직접 들고있습니다. 예를 들어, `FileReader` 는 `file_types` 속성으로 자신이 파싱할 수 있는 파일 확장자인지 검증합니다.

코드는 조금 더 복잡해졌지만, 객체지향은 기능을 확장해야할 때 진가를 발휘합니다.
...
객체지향을 지원하는 대부분의 프로그래밍 언어들은 클래스라는 문법을 제공합니다. 클래스 사용 법 중 하나인 상속을 이용하면 한번 정의해놓은 메서드를 파생 클래스에서 재사용 가능합니다. 또한 상속으로 객체간의 계층 구조를 만들 수 있습니다. 이런 계층 구조는 코드 전체 요소를 개념적으로 이해하는데 도움이 되곤 하죠.
...
객체 지향은 이렇게 다형성과 의존성 주입을 통해 조금의 코드 수정으로도 기능을 빠르게 확장가능하게 할 수 있습니다. 따라서 객체지향은 프로그램이 동적으로 다양한 객체를 필요로하며, 확장 가능하도록 설계해야 하는 경우에 적합합니다.

하지만 확장이 가능하고 유연한만큼, 처음 코드를 보는 사람들은 어렵고 헷갈리기 쉽습니다. 절차지향처럼 TOP -> DOWN 식으로 프로그램이 확정되어 전개되지 않고, 실제 실행의 다양한 입력에 따라 다양한 흐름이 만들어지기 때문에 디버깅하기도 어렵습니다. 또한 절차지향과 더불어 "상태"를 가지기 때문에 "상태"로 인해 개발자가 모든 사건을 예상하기 어렵습니다.
...
(출처: https://user.oc-static.com/upload/2017/10/19/15084493877125_C03p01ch03-procedural-vs-oop.png)
...
함수형(Functional) 프로그래밍은 상태를 갖지 않는 함수들의 연속으로 프로그래밍을 생각하는 것입니다. 상태를 갖지 않는다는 의미는, 같은 입력을 넣었을 때 언제나 같은 출력을 내보낸다는 의미 입니다. 즉 입/출력에 영향을 주는 외부 요인이 없습니다.
함수형 프로그래밍 코드에서는 한번 초기화한 변수는 변하지 않습니다. 이 때문에 대입문이 없이 프로그래밍한다라고 표현하기도 합니다. 이런 특성을 "불변성"이라고 하는데, 이 불변성을 통해 사이드 이펙트를 없애고 안정성을 얻을 수 있습니다.
...
그렇다면 왜 상태를 가지지 않으려고 할까요? 일반적으로 상태를 가지면 좋지 않기 때문입니다.

예를 들어 다음과 같이 상태를 가지는 클래스를 봅시다.
...
한편, 절차지향과 객체지향에서 다루었던 예시 코드를 함수형으로 바꾸면 다음과 같은 모양이 됩니다.

```python
def main():
	return save_data(parse_input_data(read_input_file("input_file.txt"))
```

함수를 좀 더 함수형으로 만들면 다음과 같은 모양도 가능합니다. 대부분의 함수형 프로그래밍의 코드는 사실 아래와 같은 형태입니다.
...
프로그래밍 언어 중에서는 대표적으로 '스칼라(Scala)'가 이런 함수형 프로그래밍을 따르는 언어입니다.
