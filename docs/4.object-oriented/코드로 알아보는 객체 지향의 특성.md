...
먼저 사람이라는 객체를 정의해봅니다. 사람은 "운전"이라는 행위를 해야 한다는 것을 직관적으로 알 수 있습니다. 이때 "사람" 객체는 "운전하는 것"에 대한 책임이 있습니다.
...
그런데 만약 이후에 '포르쉐' 말고도 '벤츠', 'BMW' 등 더 다양한 차를 운전하고 싶어한다면 어떻게 해야 할까요? 새로운 차량이 추가 될 때마다 `__init__()` 함수 안에 `if .. else` 문이 추가될 것입니다. 
...
이때 위 코드의 차량에 필요한 기름을 주유하기 위해 `car.fuel` 에 직접 접근하여 `+` 연산을 하고 있습니다. 또한 차랑의 남은 주유량을 퍼센트로 확인하기 위해 `car_max_fuel` 에도 접근하여 `/` 연산을 하고있습니다.
