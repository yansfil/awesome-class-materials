...
공개형 메서드는 이 클래스가 외부에 제공해주는 액션입니다. 다음 예시에서 `work()` 는 공개형 메서드 입니다.
...
```
 ...
 위 코드에서 `User` 객체는 `Job` 객체에 정적인 의존성이 있습니다. `DataEnginner` , `ProjectManager` 에는 의존성이 없습니다.
